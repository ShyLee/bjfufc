...
public class ServiceLevelAgreement extends HelpdeskEventHandlerBase {
    ...
    private int retrieveSlaOfPM(Map<String, Object> requestParameters2) {
        ...
        String matchSql = getMatchSql(fieldsValue, isEqNull);
        ...
        DataSource slaRequestSelectDS = DataSourceFactory.createDataSourceForFields(
            ...
            new String[] { "activity_type", "prob_type", "pmp_id", "eq_std", "eq_id", "site_id",
                    "bl_id", "fl_id", "rm_id", "ordering_seq" }).addSort("ordering_seq");
     ...
     */
    private String getMatchSql(JSONObject fieldValues, boolean isEqNull) {
        String matchSql = " activity_type='" + Constants.PM_ACTIVITY_TYPE + "'"
                + " AND prob_type='" + Constants.PM_PROB_TYPE + "'" + " AND ( 1=0 ";

        if (fieldValues.has("pmp_id")) {
            matchSql += " OR pmp_id='" + fieldValues.get("pmp_id") + "' ";
        }
        ...
        if (fieldValues.has("site_id") && !"".equals(fieldValues.get("site_id"))) {
            matchSql += " OR site_id='" + fieldValues.get("site_id") + "' ";
        }
        if (fieldValues.has("bl_id") && !"".equals(fieldValues.get("bl_id"))) {
            matchSql += " OR bl_id='" + fieldValues.get("bl_id") + "' ";
        }
        if (fieldValues.has("fl_id") && !"".equals(fieldValues.get("fl_id"))) {
            matchSql += " OR ( bl_id='" + fieldValues.get("bl_id") + "' " + " AND fl_id='"
                    + fieldValues.get("fl_id") + "' " + ")";
        }
        if (fieldValues.has("rm_id") && !"".equals(fieldValues.get("rm_id"))) {
            matchSql += " OR ( bl_id='" + fieldValues.get("bl_id") + "' " + " AND fl_id='"
                    + fieldValues.get("fl_id") + "' " + " AND rm_id='" + fieldValues.get("rm_id")
                    + "' " + ")";
        }
        if (!isEqNull) {
            if (fieldValues.has("eq_id") && !"".equals(fieldValues.get("eq_id"))) {
                matchSql += " OR eq_id='" + fieldValues.get("eq_id") + "' ";
            }
            if (fieldValues.has("eq_std") && !"".equals(fieldValues.get("eq_std"))) {
                matchSql += " OR eq_std='" + fieldValues.get("eq_std") + "' ";
            }
        }
        matchSql = matchSql + " ) ";
        return matchSql;
    ...
    private void getResponseParametersFromPrimaryKey() {
        ...
        Object[] response = selectDbValues(this.context, Constants.SLA_RESPONSE_TABLE,
            Constants.SLA_PARAMETER_NAMES, where);
        ...
        for (int i = 0; i < Constants.SLA_PARAMETER_NAMES.length; i++) {
            // booleans
            if (Constants.SLA_PARAMETER_NAMES[i].startsWith("auto")
                    || Constants.SLA_PARAMETER_NAMES[i].startsWith("time")
                    || Constants.SLA_PARAMETER_NAMES[i].equals("ordering_seq")
                    || Constants.SLA_PARAMETER_NAMES[i].equals("allow_work_on_holidays")
                    || Constants.SLA_PARAMETER_NAMES[i].equals("notify_requestor")
                    || Constants.SLA_PARAMETER_NAMES[i].equals("default_duration")) {
                if (response[i] == null) {
                    json.put(Constants.SLA_PARAMETER_NAMES[i], 0);
                } else {
                    json.put(Constants.SLA_PARAMETER_NAMES[i], response[i]);
                }
            } else { // other response parameters
                if (response[i] == null) {
                    json.put(Constants.SLA_PARAMETER_NAMES[i], "");
                } else {
                    json.put(Constants.SLA_PARAMETER_NAMES[i], response[i]);
                }
            }
