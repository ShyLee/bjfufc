...
function ASBT_getYearMonthOfDate(dateString){
    var yearMonth = "";
...
function IdCardValidate(idCard) {  
    idCard = identitrim(idCard.replace(/ /g, ""));  
    if (idCard.length == 15) {  
...
function maleOrFemalByIdCard(idCard){  
    idCard = identitrim(idCard.replace(/ /g, ""));// 对身份证号码做处理。包括字符间有空格。  
    if(idCard.length==15){  
