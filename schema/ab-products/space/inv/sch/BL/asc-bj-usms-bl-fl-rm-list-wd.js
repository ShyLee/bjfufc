...
var abScRptRmInvByFl =  View.createController('abScRptRmInvByFlController', {
	...
	afterViewLoad: function(){
	        ...
	        this.sbfFilterPanel.onKeyPressFilter=function(){  
	        	abScRptRmInvByFl.sbfFilterPanel_onShow(); 
...
function onPrTreeClick(){
	...
	var prName=currentNode.data['property.name'];
	var title=""
	if(prName!=""){
		title = prName;
	}else{
    ...
    var restriction = new Ab.view.Restriction();
    facultySumGrid.refresh(restriction);
    facultySumGrid.setTitle("房间列表 :"+title);
}
...
function afterGeneratingTreeNode(treeNode){
    ...
    if (treeNode.level.levelIndex == 1) {
        ...
        labelText2 = "<span class='" + treeNode.level.cssClassName + "'>" + prName + "</span> ";
