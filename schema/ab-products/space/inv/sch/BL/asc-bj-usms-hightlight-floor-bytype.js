...
var controller = View.createController('hlRmByCatAndTypeController', {
	...
	abSpHlRmByRmcatRmtype_DrawingPanel_onShowDwgView:function(){
		   if (this.abSpHlRmByRmcatRmtype_DrawingPanel.dwgLoaded)
		   {
		   	  var recValue = this.abSpHlRmByRmcatRmtype_DrawingPanel.getRecValues(0);
	...
	abSpHlRmByRmcatRmtype_filterConsole_onClear:function(){
		this.abSpHlRmByRmcatRmtype_BlTree.refreshClearAllFilters();
	}
});
...
function onFlTreeClick(ob){
    ...
    var title = String.format(getMessage('drawingPanelTitle2'), blName + "-" + flId);
...
function onRmCatTreeClick(ob){
    ...
    var rmCatName = currentNode.data['rmcat.rmcat_name'];
    var blId = currentNode.parent.parent.data['bl.bl_id'];
    var blName = currentNode.parent.parent.data['bl.name'];
    ...
    var drawingPanel = View.panels.get('abSpHlRmByRmcatRmtype_DrawingPanel');
    var title = String.format(getMessage('drawingPanelTitle3'), blName + "-" + flId, rmCatName);
...
function onRmTypeTreeClick(ob){
    ...
    var rmTypeName = currentNode.data['rmtype.rmtype_name'];
    var blId = currentNode.parent.parent.parent.data['bl.bl_id'];
    var blName = currentNode.parent.parent.parent.data['bl.name'];
    var flId = currentNode.parent.parent.data['fl.fl_id'];
    var rmCat = currentNode.parent.data['rm.rm_cat'];
    var rmCatName = currentNode.parent.data['rmcat.rmcat_name'];
    var restriction = new Ab.view.Restriction();
    ...
    var drawingPanel = View.panels.get('abSpHlRmByRmcatRmtype_DrawingPanel');
    var title = String.format(getMessage('drawingPanelTitle4'), blName + '-' + flId, rmCatName, rmTypeName);
    drawingPanel.currentHighlightDS = 'ds_ab-sp-hl-rm-by-rmcat-rmtype_drawing_rmHighlight';
    displayFloor(drawingPanel, currentNode, title);
...
function afterGeneratingTreeNode(treeNode){
    if (treeNode.tree.id != 'abSpHlRmByRmcatRmtype_BlTree') {
        return;
    }
    var labelText1 = "";
    if (treeNode.level.levelIndex == 0) {
        ...
        labelText1 = "<span class='" + treeNode.level.cssClassName + "'>" +blName +"["+blId +"]</span> ";
