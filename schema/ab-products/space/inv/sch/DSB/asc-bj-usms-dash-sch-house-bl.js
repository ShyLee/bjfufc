...
View.createController('ascBjUsmsDashHouseBlController', {
    ...
    afterViewLoad: function(){
        ...
        panle.addParameter('rmCatZG', "rm.rm_cat = '职工宿舍'");
        panle.addParameter('rmCatXS', "rm.rm_cat = '学生宿舍'");
        panle.addParameter('rmCatZGXS', "rm.rm_cat in ('职工宿舍', '学生宿舍')");
        panle.addParameter('blIdRes', "bl_id != '金鑫公寓'");
    },
...
function showBlList(row){
    ...
    if (blUse == '学校总计') {
        ...
        if (blUse == '教学办公') {
            ...
            if (blUse == '教工宿舍') {
                blGrid.addParameter('blCatRes', ") and  bl.bl_id in (select rm.bl_id from rm, rmcat where rm.rm_cat = rmcat.rm_cat and rm.rm_cat = '职工宿舍' ");
                blGrid.refresh();
            }
            else 
                if (blUse == '学生宿舍') {
                    ...
                    blGrid.addParameter('blCatRes', " and  use1 not in ('TE', 'TEOFF', 'TEASS', 'TELV', 'OFF', 'LVOFF')) and not exists (select 1 from rm where rm.bl_id = bl.bl_id and rm.rm_cat in ('职工宿舍', '学生宿舍') and bl_id != '金鑫公寓'");
                    blGrid.refresh();
                }
