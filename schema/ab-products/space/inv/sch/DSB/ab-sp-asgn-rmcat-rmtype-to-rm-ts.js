...
jQuery().ready(function() {
		 ...
		 jQuery("#abSpAsgnRmcatRmTypeToRm_filterConsole input[class*='inputValueChg']").on("change",function(e){
			   ...
			   if(jQuery(this).val().trim()==''){
				   ...
				   switch(eventSrc)
				   {
				   case 'abSpAsgnRmcatRmTypeToRm_filterConsole_bl.name': //判断当前的事件源 是哪一个文本框 case里面写的是 当前文本框的id
					   jQuery("#abSpAsgnRmcatRmTypeToRm_filterConsole_rm\\.bl_id").val('');
					     break;
				   case 'abSpAsgnRmcatRmTypeToRm_filterConsole_rmcat.rmcat_name':
					   jQuery("#abSpAsgnRmcatRmTypeToRm_filterConsole_rm\\.rm_cat").val('');
				     break;
...
//        this.abSpAsgnEmToRm_emSelect.enableSelectAll(false);
    	...
    	if(employeeDivision!=undefined){
    		var bu_id= AUSC_getEmployeeDivisionIdByEmId(View.user.name).organization.dvType;
    		 if (ascBjUsmsConstantControl.AUSC_DvIsJXKY(bu_id)){
    			 //this.abSpAsgnRmcatRmTypeToRm_rmcatTree.addParameter("buparam"," rmtype_bu in ('JX','JXJG')");
    			 this.abSpAsgnRmcatRmTypeToRm_rmcatTree.addParameter("buparam", " 1 = 1 ");
    		 }else{
    			 //this.abSpAsgnRmcatRmTypeToRm_rmcatTree.addParameter("buparam"," rmtype_bu in ('JG','JXJG')");
    			 this.abSpAsgnRmcatRmTypeToRm_rmcatTree.addParameter("buparam", " 1 = 1 ");
    		 }
        ...
        var mainlayout=View.getLayoutManager('main');
   	 	var nestedWest=View.getLayoutManager('nested_west');
   	 	var nestedCenter=View.getLayoutManager('nested_center');
   	 	var nestedNorth=View.getLayoutManager('nested_north');
	 	...
	 	var toolBarPanel;
	 	if(valueExistsNotEmpty(toolBarRegion)){
	 		toolBarPanel=toolBarRegion.layoutManager.getRegionPanel(toolBarRegion.region);
	 	}
   		
	 	if(valueExistsNotEmpty(dashboardTabsRegion)){
	 		dashboardTabsPanel=dashboardTabsRegion.layoutManager.getRegionPanel(dashboardTabsRegion.region);
	 	}
   	 	var expandView = this.abSpAsgnRmcatRmTypeToRm_drawingPanel.actions.get('showDwgView');
   	 
    	 jQuery(expandView.button.el.dom).toggle(function(){
    		 mainlayout.collapseRegion('north');
    		 mainlayout.collapseRegion('west');
    		 nestedWest.collapseRegion('north');
    		 nestedCenter.collapseRegion('south');
    		 //nestedNorth.collapseRegion('east');
        	 expandView.setTitle("返回");
        	 if(toolBarPanel) toolBarPanel.collapse(false);
        	 if(dashboardTabsRegion)  dashboardTabsRegion.layoutManager.collapseRegion("north");
        	
    	 },function(){
 
        	 mainlayout.expandRegion('north');
          	 mainlayout.expandRegion('west');
    		 nestedWest.expandRegion('north');
    		 nestedCenter.expandRegion('south');
    		 //nestedNorth.expandRegion('east');
        	 expandView.setTitle("全屏");
        	 if(toolBarPanel) toolBarPanel.expand(false);    	
        	 if(dashboardTabsRegion) dashboardTabsRegion.layoutManager.expandRegion("north");
...
//		   }
//	},
	abSpAsgnRmcatRmTypeToRm_blTree_onEdit:function(){
		View.openDialog('ab-sp-asgn-rmcat-rmtype-to-rm-noimg.axvw', null, false, {
            width: 900,
            height: 500,
            closeButton: false
        });
