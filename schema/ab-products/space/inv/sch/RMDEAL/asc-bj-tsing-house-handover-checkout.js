var controller=View.createController('HandoverForm',{
	...
	handoverDetailPanel_onBtnSave: function(){
		var dealRmDS=View.dataSources.get('ts_dealrm_ds');
		var dealRmId=this.handoverDetailPanel.getFieldValue('ts_handover.deal_rm_id');
	
		var restriction = new Ab.view.Restriction();
		restriction.addClause("ts_deal_rm.deal_rm_id",dealRmId,"=");
		var record=dealRmDS.getRecord(restriction);
		record.setValue('ts_deal_rm.status',0);
		dealRmDS.saveRecord(record);
		
		//更改rm表中的房屋状态
		var rmDS=View.dataSources.get('rm_ds');
		var blId=this.handoverDetailPanel.getFieldValue('ts_handover.bl_id');
		var flId=this.handoverDetailPanel.getFieldValue('ts_handover.fl_id');
		var rmId=this.handoverDetailPanel.getFieldValue('ts_handover.rm_id');
		var rm_status_to=this.handoverDetailPanel.getFieldValue('ts_handover.rm_status_to');
		var restrictionRm = new Ab.view.Restriction();
		restrictionRm.addClause("rm.bl_id",blId,"=");
		restrictionRm.addClause("rm.fl_id",flId,"=");
		restrictionRm.addClause("rm.rm_id",rmId,"=");
		var recordRm=rmDS.getRecord(restrictionRm);
		recordRm.setValue('rm.ruzhu_status',rm_status_to);
		rmDS.saveRecord(recordRm);
		var a;
	},
