...
var abDwgChangesByBlCtrl = View.createController('abDwgChangesByBlCtrl',{
	...
	afterViewLoad:function(){
		...
		jQuery('#abDwgChangesByBlFilter input').each(function(){
			jQuery(this).keydown( function (e) {
				if(e.keyCode == 13){
					this.blur();
					abDwgChangesByBlCtrl.abDwgChangesByBlFilter_onShow();
					this.focus();					
				}
			});
	...
	abDwgChangesByBlFilter_onShow: function(){
		var paramSiteId = "1 = 1";
		var paramBlId = "1 = 1";
		var siteName = this.abDwgChangesByBlFilter.getFieldValue("site.name");
		if (valueExistsNotEmpty(siteName)){
			paramSiteId = "site.name like '%" + siteName + "%'";
		}
		var blName = this.abDwgChangesByBlFilter.getFieldValue("bl.name");
		if (valueExistsNotEmpty(blName)){
			paramBlId = "bl.name  like '%" + blName + "%'";
		}
		this.blId = null;
		this.flId = null;
