...
function addTotalRow(parentElement){
	...
	var totalRecordNum= this.rows.length;
    ...
    addColumn(gridRow, 1, getMessage('total')+":"+totalRecordNum,"left");
	// column 2: empty	
    addColumn(gridRow, 1);
	// column 3: empty	
    addColumn(gridRow, 1);
 ...
 * @param {String} text
 */
function addColumn(gridRow, count, text,align){
    for (var i = 0; i < count; i++) {
        ...
        if (text) {
            ...
            if(align){
            	gridCell.style.textAlign=align;
            }
            gridCell.style.fontWeight = 'bold';
            gridCell.style.fontSize="13px";
        }
