var reviewMyRequestController =  View.createController("reviewMyRequestController",{
	status : '',
	requestDateFrom: '',
	requestDateTo: '',
	afterViewLoad: function(){
	        this.requestReportGrid.addParameter("activityType", ZZFCommonController.activityType);
	        View.getControlsByType(parent, 'tabs')[0].appliedPanel=this.requestReportGrid;
	},	
...
function SaveConsoleParameters(){
    ...
    var sqId = record.getValue('bh_zzf_apply.zzf_sq_id');
    var logId = record.getValue('activity_log.activity_log_id');
    View.getControlsByType(parent, 'tabs')[0].requestNumber=sqId;
    View.getControlsByType(parent, 'tabs')[0].activityLogId=logId;
	reviewMyRequestController.saveRequestConsoleParameters();
}
