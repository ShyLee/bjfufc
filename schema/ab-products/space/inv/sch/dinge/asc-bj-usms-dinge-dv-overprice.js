...
var controller = View.createController('mySingleJobReportController', {
	...
	ruleId: 'AbSpaceRoomInventoryBAR-SchoolJobService-overPriceThread',
	jobId: '',
	...
	startTaskJob: function() {
		...
		var time = year+","+month;
		if(year==""||month==""){
			View.showMessage("请输入年份和月份！");
			return;
		}else{
			if(!isNaN(year+month)){
				var message="开始计算超额费用,点击【确定】计算【"+year+"】年超额费用,点击【取消】停止计算!";
				this.startJob(message,time);
			}
			else{
				View.showMessage("年份和月份必须为数字！");
			}
		}
	},
	startJob:function(message,time){
		if(confirm(message)){
			var jobId = '';
			this.params.length=0;
			this.params.push(time);
			this.paramNum=1;
