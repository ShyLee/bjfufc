...
var image_path = "/" + current_view_path.substring(current_view_path.indexOf('archibus'), current_view_path.lastIndexOf('asc-bj-usms-proc-def-response-para-tab.axvw')) + "graphics/";
...
function getHelperRules(){
    ...
    if (result.code == 'executed') {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            var step = res[i].step;
            var rejectStep = res[i].step_rejected;
            ...
            li.setAttribute("step_rejected", rejectStep);
...
//        document.getElementById("helpdesk_sla_steps.em_id").value = "";
    ...
    //2013-03-15 yan Add
    if (dropItem.getAttribute("step_rejected") != null) {
        document.getElementById("helpdesk_sla_steps.step_rejected").value = dropItem.getAttribute("step_rejected");
    }
    else {
        document.getElementById("helpdesk_sla_steps.step_rejected").value = "";
    }
    //2013-03-15 yan End
...
//    $("helpdesk_sla_steps.role").value == "") {
    ...
    //2013.1.16 添加拒绝回退步骤 --begin
    rejectStep = document.getElementById("helpdesk_sla_steps.step_rejected").value;
    dropItem.setAttribute("step_rejected", rejectStep);
  //--end
    
    //2012-12-11 guo add 1 --begin
...
function saveForm(){
    ...
    for (var i = 0; i < uls.length; i++) {
        ...
        for (var x = 0; x < lis.length; x++) {
            ...
            //添加拒绝回退步骤2013.1.16-begin
            if (lis[x].getAttribute("step_rejected") != null && lis[x].getAttribute("step_rejected") != "") 
            	xml += ' step_rejected="' + encodeURIComponent(lis[x].getAttribute("step_rejected")) + '"';
            //end
...
function afterSelectType(){
	var selectElement = document.getElementById("helpdesk_sla_steps.step_rejected");
	selectElement.length = 0;
	var uls = document.getElementsByTagName("ul");
	  ...
	  for (var i = 0; i < uls.length; i++) {
    	  ...
    	  for (var j = 0; j< lis.length; j++) {
    		  selectElement.length = selectElement.length + 1;
    		  var option =new Option(lis[j].getAttribute("step"), lis[j].getAttribute("step"));
    		  selectElement.options[j + 1] = option;
    	  }
	  }
}