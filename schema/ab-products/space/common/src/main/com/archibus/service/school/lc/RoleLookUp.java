...
public class RoleLookUp extends EventHandlerBase {
    ...
    public List<String> getDvHeadListForZzf(final EventHandlerContext context) {
        final String role = (String) context.getParameter("role");
        String activityLogId =context.getString("activity_log.activity_log_id");
        this.log.info("role is " + role);
        Integer sc_zzfcard_id=new Integer(0);
        final List cardIdList =
                selectDbRecords(context, "activity_log", new String[] { "zzfcard_id" },
                    "activity_log.activity_log_id ='" + activityLogId + "'");
        if(cardIdList !=null && cardIdList.size()>0){
            for (final Iterator it = cardIdList.iterator(); it.hasNext();) {
                final Object[] record = (Object[]) it.next();
                sc_zzfcard_id=  (Integer) record[0];
            }
        }
        ...
        String dvId="";
            ...
            final List dvIdList =
                    selectDbRecords(context, "sc_zzfcard", new String[] { "dv_name" },
                        "sc_zzfcard.card_id ='" + sc_zzfcard_id + "'");
            
            for (final Iterator it = dvIdList.iterator(); it.hasNext();) {
                final Object[] record = (Object[]) it.next();
                dvId= (String) record[0];
            }
 
        final List<String> emlist = new ArrayList<String>();
        final List records =
                selectDbRecords(context, "sc_role_em", new String[] { "em_id", "role" },
                    "sc_role_em.role ='" + role + "' and dv_id = '"+dvId+"'");
