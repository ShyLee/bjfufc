 ...
 * @author ZhaoYongLi
 */
public class HouseServiceHandler extends SchoolJobService {
    ...
    public void checkDiffOfActualAndCalculateRent(final String yearmonth,
        ...
        try {
            ...
            checkYearMonth(yearmonth, this.yearMonth);
     ...
     */
    private void checkYearMonth(final String yearmonth, final List<String> yearMonth) {
        this.log.info("正在检查前导入数据的月份是否匹配...");
        if (null == yearMonth) {
            throw new ExceptionBase(String.format("当前导入文件的年月为空！"));
        }
        for (final String str : yearMonth) {
            if (str.equals("") || null == str) {
                this.log.error("当前导入年月字段存在[空值]！");
                throw new ExceptionBase(String.format("当前导入文件年月存在[空值]！"));
            }
            
            if (!yearmonth.equals(str)) {
                this.log.error("当前导入文件的年月字段[与检查年月不一致]！");
                throw new ExceptionBase(String.format("当前导入文件的年月字段[与检查年月不一致]！"));
            }
