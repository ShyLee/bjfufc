...
//        updateDbCounts(fieldNames, updatedRecords);
    ...
    resetAssignment1(drawingPanel, grid);
}
...
function resetAssignment1(drawingPanel, grid){
//	grid.removeRows(0);
	drawingPanel.clearAssignCache(false);
	drawingPanel.refresh();
//	grid.update();
 ...
 * @param {Object} grid
 */
function ABSC_rmFlTreeClickHandler(currentNode, drawingPanel, grid){
    var blId = getValueFromTreeNode(currentNode, 'bl.bl_id');
    var flId = getValueFromTreeNode(currentNode, 'rm.fl_id');
    var dwgName = getValueFromTreeNode(currentNode, 'rm.dwgname');
    var dcl = new Ab.drawing.DwgCtrlLoc(blId, flId, null, dwgName);
    if (grid.gridRows.length > 0) {
        var message = getMessage('confirmMessage');
        View.confirm(message, function(button){
...
function drawingRoomClickHandler2(pk, selected, grid, field1, value1, field2, value2,field3, value3,field4, value4){
    var rec = new Ab.data.Record();
    var name = pk[0] + "-" + pk[1] + "-" + pk[2];
    	rec.setValue("composite.loc", name);
    	rec.setValue(field1, value1);
    ...
    if (field2) {
        ...
        rec.setValue('rm.dv_id', value3);
        rec.setValue('rm.dp_id', value4);
...
function drawingRoomClickHandler2(pk, selected, grid, field1, value1, field2, value2,field3, value3,field4, value4, field5, value5,field6, value6,field7, value7,field8, value8){
    var rec = new Ab.data.Record();
    var name = pk[0] + "-" + pk[1] + "-" + pk[2];
	...
	if (field4) {
        rec.setValue(field4, value4);
    }
	if (field5) {
        rec.setValue(field5, value5);
    }
	if (field6) {
        rec.setValue(field6, value6);
	}
	if (field7) {
		rec.setValue(field7, value7);
	}
	if (field8) {
		rec.setValue(field8, value8);
	}
    // Find the existing grid row and remove it, if it exists
    var bFound = false;
    for (var i = 0; i < grid.gridRows.length && !bFound; i++) {
...
function drawingRoomDvClickHandler2(pk, selected, grid, field1, value1, field2, value2){
	var rec = new Ab.data.Record();
	var name = pk[0] + "-" + pk[1] + "-" + pk[2];
...
function resetAssgnDvColor(hpfieldName, hpRecords, field1, val1){
	for (var i = 0; i < hpRecords.length; i++) {
		var record = hpRecords[i];
		if (record.getValue(field1) == val1) {
			var hpval = record.getValue(hpfieldName);
			if (hpval.length) {
				color = gAcadColorMgr.getRGBFromPattern(hpval, true);
				gAcadColorMgr.setColor(field1, val1, color);
				return;
			}
		}
	}
