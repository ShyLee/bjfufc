...
Ab.navigator.Navigator = Ab.view.Component.extend({
    ...
    showHelp: function(item, params) {
        ...
        if (params.record.getHelpSystem().toLowerCase() === 'system') {
            baseHelpUrl = View.systemAdministrationHelpLink;
        }
		var helpLink = baseHelpUrl + "#.." + params.record.getHelpLink();
...
Ab.navigator.Toolbar = Ab.view.Component.extend({
    ...
    doLayout: function() {
		...
		var titleLink = 'http://www.bjfu.edu.cn';
	...
	getMenuContainer: function() {
		...
		var tblHTML = 
			'<table class="titlebar-menu-container" id="titlebarMenuContainer">' +
			...
			'<td style="text-align:right;"><a id="titleBarHelpLink">' + View.getLocalizedString(this.z_HELP_MESSAGE) + '</a></td></tr>' +
			'<tr><td colspan="3">' +
	...
	getMenuTooltip: function() {
		...
		var tblHTML = '<div class="hovertip" id="userOptionsContainer" target="titleBarUserLink">' +
			...
			'<tr><td>' + View.project.title + View.getLocalizedString(this.z_PROJECT_LABEL) + '</td></tr>' +
			'<tr><td>' + View.user.role + View.getLocalizedString(this.z_ROLE_LABEL) + '</td></tr>' +
	...
	onSearchBoxSelect: function(elem) {
		//var searchBox = jQuery('#searchText');
	...
	onSearchBoxKeyPress: function(event) {
		...
		if (charCode == 13) {
			...
			if (searchText.length > 0) {
				var userLocale = View.user.locale;
				var userLang = ENGLISH;
				if (userLocale && userLocale.substr(0,2) != ENGLISH) {
					userLang = userLocale.substr(0,2);
				}

				var taskTitleColumn = 'afm_ptasks.task_id';
				if (userLang != ENGLISH) {
					taskTitleColumn = 'afm_ptasks.task_' + userLang;
				}
				...
				View.openDialog('ab-navigator-view-search.axvw', null, false, {
					...
					searchText: taskTitleColumn + ";" + searchText
				});
			}
		}
	},
...
if(typeof(AjaxDataService)!="undefined"){
	AjaxDataService.getUserName({
		   callback:function(data){
			
			if(data!=''){
				jQuery('#titleBarUserLink').html(View.user.name+"|"+data);
			}
		   }
		  });
