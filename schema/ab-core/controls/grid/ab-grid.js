...
Ab.grid.Grid = Ab.view.Component.extend({
	...
	constructor: function(id, configObject) {
		...
		configObject.setConfigParameter('maxCellContentWidth',8);
	 ...
	 */
	createCellContent: function(row, column, cellElement, colorVal,realLength) {
		var content = "";
		...
		if (column.type == 'text' || column.type == 'number' || column.type == 'date' || column.type == 'time') {
			...
			if (value.length > this.maxCellContentWidth && realLength>6) {
                value = value.replace(/\r\n/g, "<br/>");
                if(!valueExistsNotEmpty(column.width)){
		    ...
		    content = "<a href='javascript: //'>" + Ext.util.Format.ellipsis(value, 50) + "</a>";
		    }
			
			if (value.length > this.maxCellContentWidth && !valueExistsNotEmpty(column.width) && realLength>6 && column.javaType != "java.sql.Date" ) {
